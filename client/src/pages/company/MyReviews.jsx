...
import { FiStar, FiUser, FiTrendingUp, FiAward, FiFilter, FiX } from 'react-icons/fi';
...
const MyReviews = () => {
  ...
  const fetchReviews = async () => {
    try {
      const response = await companyAPI.getMyReviews();
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8 pb-8">
        ...
        <PageHeader
          title="My Reviews"
          subtitle="See what freelancers think about working with you"
          breadcrumbs={[
          ...
          <>
            ...
            {reviews.length === 0 ? (
              <EmptyState
                ...
                description="Complete jobs and receive reviews from freelancers you've worked with"
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredReviews.map((review) => (
                    <div
                      key={review._id}
                      onClick={() => navigate(`/company/jobs/${review.job?._id}`)}
                      className="cursor-pointer transition-transform hover:scale-[1.02]"
                    >
                      <ReviewCard
                        review={review}
                        reviewerType="worker"
                        reviewerName={review.workerInfo?.fullName || 'Freelancer'}
                        reviewerLogo={review.workerInfo?.profilePicture}
